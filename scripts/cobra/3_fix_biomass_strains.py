...
if __name__=="__main__":
    # Reference model
    strain = "redball"
    model_file = Path(f"models/draft/v0.0.1/hlacustris.xml")
    reference = cobra.io.read_sbml_model(model_file)
    ref_rxns = {rxn.id for rxn in reference.reactions}

    # Load the model
    strain = "nies"
    model_file = Path(f"models/draft/v0.0.2/{strain}.xml")
    model = cobra.io.read_sbml_model(model_file)
    reactions_df = pl.read_excel(
        f"models/draft/v0.0.2/{strain}.xlsx",
    ...
    # Save the model
    out_dir = Path("models/draft/v0.0.3/nies144/")
